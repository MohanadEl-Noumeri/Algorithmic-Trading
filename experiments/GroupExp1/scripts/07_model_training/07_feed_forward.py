...
# Load & Normalize Data
# -----------------------------
def load_data(file_path, features, target_col, scaler):
    """Load parquet and return normalized X, y."""
    ...
    logger.info(f"  Class balance: Down={np.mean(y == 0):.1%}, Up={np.mean(y == 1):.1%}")

    return torch.tensor(X), torch.tensor(y)


# Load datasets
train_X, train_y = load_data(os.path.join(DATA_PATH, "train.parquet"), FEATURES, TARGET, scaler)
val_X, val_y = load_data(os.path.join(DATA_PATH, "val.parquet"), FEATURES, TARGET, scaler)
test_X, test_y = load_data(os.path.join(DATA_PATH, "test.parquet"), FEATURES, TARGET, scaler)
...
# -----------------------------
# Model (Professor's MLP style)
# -----------------------------
class MLP(nn.Module):
    """Simple 2-hidden-layer MLP for binary classification."""
...
plt.tight_layout()
plt.savefig(os.path.join(MODEL_PATH, "training_metrics.png"), dpi=150)
logger.info(f"\nSaved plot to {MODEL_PATH}/training_metrics.png")
